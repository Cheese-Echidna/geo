...
pub fn delaunay_triangulation(app: &App, model: &Model, corners: bool) -> Vec<(Point, Point, Point)> {
    ...
    let mut x = model.points.iter().map(|x| Point64::from(x.clone())).collect::<Vec<Point64>>();

    if corners {
        app.window_rect().corners_iter().for_each(|pos| {
            let p = Point64::from(pos);
            x.push(p);

            let p = Vec2::from(pos);
            let point = Point::new(p, lin_srgb(0.0, 0.0, 0.0));
            y.push(point);
        });
    };
    let triangulation = delaunay::triangulate(&x);
    triangulation.triangles.windows(3).step_by(3).map(|slice| {
        let i0 = slice[0];
        let i1 = slice[1];
        let i2 = slice[2];
        (y[i0].clone(), y[i1].clone(), y[i2].clone())
    }).collect::<Vec<(Point, Point, Point)>>()
}

/// This function returns a vec of polygons with the points
pub fn voronoi_diagram(app: &App, model: &Model) -> Vec<(Vec<Vec2>, LinSrgb<f32>)> {
    let bounds_min = (app.window_rect().x.start as f64, app.window_rect().y.start as f64);
    let bounds_max = (app.window_rect().x.end as f64, app.window_rect().y.end as f64);

    let points = model.points.iter()
        .map(|p| (p.pos.x as f64, p.pos.y as f64))
        .collect::<Vec<(f64, f64)>>();

    if let Some(diagram) = VoronoiDiagram::<voronator::delaunator::Point>::from_tuple(&bounds_min, &bounds_max, &points) {
        diagram.cells().into_iter().enumerate()
            .map(|(i, x)| {
                let colour = model.points[i].clone().colour;
                let poly = x.points().into_iter().map(|y| {
                    Vec2::new(y.x as f32, y.y as f32)
                }).collect::<Vec<Vec2>>();
                (poly, colour)
            }
            ).collect::<Vec<(Vec<Vec2>, LinSrgb<f32>)>>()
    } else {
        vec![]
    }
}
...
impl From<&Point64> for robust::Coord<f64> {
    fn from(p: &Point64) -> robust::Coord<f64> {
        robust::Coord::<f64> { x: p.x, y: p.y }
    }
}

impl From<(f64, f64)> for Point64 {
    fn from((x, y): (f64, f64)) -> Self {
        Point64 { x, y }
    }
}

impl From<[f64; 2]> for Point64 {
    fn from([x, y]: [f64; 2]) -> Self {
        Point64 { x, y }
    }
}

impl From<Point64> for (f64, f64) {
    fn from(pt: Point64) -> Self {
        (pt.x, pt.y)
    }
}

impl From<Point64> for [f64; 2] {
    fn from(pt: Point64) -> Self {
        [pt.x, pt.y]
    }
}
...
impl Triangulation {
    ...
    fn legalize(&mut self, a: usize, points: &[Point64], hull: &mut Hull) -> usize {
...
impl Hull {
    fn new(n: usize, center: Point64, i0: usize, i1: usize, i2: usize, points: &[Point64]) -> Self {
    ...
    fn hash_key(&self, p: &Point64) -> usize {
    ...
    fn hash_edge(&mut self, p: &Point64, i: usize) {
        let key = self.hash_key(p);
        self.hash[key] = i;
    }

    fn find_visible_edge(&self, p: &Point64, points: &[Point64]) -> (usize, bool) {
...
fn calc_bbox_center(points: &[Point64]) -> Point64 {
...
fn find_closest_point(points: &[Point64], p0: &Point64) -> Option<usize> {
...
fn find_seed_triangle(points: &[Point64]) -> Option<(usize, usize, usize)> {
...
fn handle_collinear_points(points: &[Point64]) -> Triangulation {
    let Point64 { x, y } = points.first().cloned().unwrap_or_default();
...
pub fn triangulate(points: &[Point64]) -> Triangulation {
